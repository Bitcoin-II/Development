             ...
             std::optional<std::string> expected_prv = std::nullopt, std::optional<std::string> expected_pub = std::nullopt, int desc_index = 0)
{
    ...
    const auto max_sat2_maxsig{parse_priv->MaxSatisfactionWeight(true)};
    const auto max_sat2_nonmaxsig{parse_priv->MaxSatisfactionWeight(false)};
    BOOST_CHECK(max_sat2_nonmaxsig <= max_sat2_maxsig);
