# BitcoinII RocksDB Storage Backend

BitcoinII replaces BitcoinII Core's LevelDB database backend with RocksDB.

RocksDB is used for the node's persistent key/value databases, including the
block index, chainstate, and enabled optional indexes. BitcoinII applies
...
There is not currently any auto-migration implementation in BitcoinII to
automatically convert current BitcoinII Core LevelDB databases to RocksDB
...
BitcoinII is developed and tested against the RocksDB version specified by
the BitcoinII Depends build system.

Users building BitcoinII from source are strongly encouraged to build the
required dependencies using `depends/` rather than substituting arbitrary
system versions of RocksDB, LZ4, or Zstandard.

The versions selected by Depends are the versions against which BitcoinII
is developed and tested.

## Automatic Database Cache

When `-dbcache` is not explicitly configured, BitcoinII selects the total
...
If physical-memory detection is unavailable, BitcoinII falls back to a
...
BitcoinII logs the selected cache size and whether the standard or
high-end database profile is active during startup.

## Workload-Specific RocksDB Profiles

BitcoinII does not configure every RocksDB database identically.
...
standard BitcoinII Core-compatible `dbcache` option.

For example, in `bitcoinII.conf`:

dbcache=4096

or from the command line:

bitcoinII-d -dbcache=4096

The value is specified in MiB.

Providing an explicit dbcache value disables automatic selection of the
total database cache. BitcoinII will continue to distribute that cache
...
BitcoinII enables compression within RocksDB.
...
BitcoinII also enables database checksums and workload-specific Bloom
...
RocksDB compression should not be confused with BitcoinII compressed block
storage.

BitcoinII block payloads stored in blk*.dat are handled by the BitcoinII
block-storage layer, not RocksDB. BitcoinII independently compresses block
...
See the BitcoinII disk-storage comparison document for measured storage
results.
...
BitcoinII automatically accounts for enabled indexes when distributing the
...
BitcoinII reports its selected database configuration in debug.log.
...
BitcoinII intentionally does not expose the full RocksDB option set through
bitcoinII.conf.
...
BitcoinII instead provides automatic workload-specific profiles while
