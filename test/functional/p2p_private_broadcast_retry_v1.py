...
* Get one successful connection to a Tor peer (.onion) so that bitcoinIId assumes the configured
...
class P2PPrivateBroadcastRetryV1(BitcoinIITestFramework):
    ...
    def run_test(self):
        ...
        # The destinations behind the -proxy= don't actually support v2. When bitcoinIId runs with -v2transport=1
        ...
        self.log.info("Opening a connection to a Tor addresses, so bitcoinIId considers -onion= is a real Tor proxy")
